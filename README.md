...
> 有时你需要回到之前的提交查看，或者撤销某次出问题的提交，这时就可以使用 `git checkout`。

```markdown
git checkout <HASH>
```

> 使用 `git log` 显示提交历史，找到你想回去查看的提交，记下提交哈希的前 6 位字符。执行 `git checkout <HASH>` 切换到该提交，Git 会显示有关 HEAD 分离状态的警告。

```markdown
git log
```

> 检查源文件的内容，确认它已经回到指定的状态。之后，使用 `git checkout main` 返回到最新版本。
...
> 使用 `git revert HASH` 创建一个新提交，将文件恢复到某个提交之前的状态。适合团队开发，因为它不会删除提交历史，只会增加一个新提交。

```markdown
git revert <HASH>
```

> 使用 `git reset HASH` 将 HEAD 指针移回某个提交，但不会影响工作区内容。要完全恢复文件内容可以使用 `--hard`。注意：如果你已经推送了新提交，使用 `git reset` 可能会造成问题。

```markdown
git reset <HASH>
```

> **注意**：如果你不小心提交了包含秘密信息的文件，并且已经推送了提交，可能需要使用“强制推送”来完全删除该文件的痕迹。另外，如果文件中有密码，建议立即更改密码。
>>>>>>> 198c4d0 (update 24.10.19 --Mandy)
